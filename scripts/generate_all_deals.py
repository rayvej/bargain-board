...
# Filter out misattributed deals
deals = [
    d for d in deals
    if not (
        ('bbq' in d.get('title', '').lower() or 'cleaner' in d.get('title', '').lower() or 'battery' in d.get('title', '').lower() or 'batteries' in d.get('title', '').lower())
        and d.get('brand') in ['New Balance', 'Asics']
    )
]

print(f"Base catalog count: {len(deals)}")
...
COUPONS = {
    'nike': [{
        ...
        'checkoutInstructions': 'Sign in to your Nike Member account and enter MEMBER20 at checkout.'
    }],
    'adidas': [{
        'code': 'EXTRA15',
        'discount': '15% OFF',
...
def make_deal(title, brand, retailer, gender, category, subcategory, sizes, orig, sale, url, img=None, coupons=None):
